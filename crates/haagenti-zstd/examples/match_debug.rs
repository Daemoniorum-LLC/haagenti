...
fn main() {
    ...
    let mut offset_counts: std::collections::HashMap<usize, (usize, usize)> = std::collections::HashMap::new();
    for m in &matches {
    ...
    for (offset, (count, total_len)) in offsets.iter().take(20) {
        println!("  offset={}: {} matches, total {} bytes", offset, count, total_len);
    }

    println!("\nFirst 20 matches:");
    for (i, m) in matches.iter().take(20).enumerate() {
        let src_start = m.position - m.offset;
        println!("  {}: pos={}, offset={}, len={} (src={})",
    ...
    println!("  Match coverage: {:.1}%", total_matched as f64 / data.len() as f64 * 100.0);

    // What would be ideal?
    // Pattern repeats every 161 bytes, so after position 161, every byte could be matched
    let ideal_matched = data.len() - sample.len();
    println!("\nIdeal (assuming first {} bytes are literals):", sample.len());
    println!("  Should match: {} bytes", ideal_matched);
    println!("  Should have ~{} long matches at offset {}", (data.len() - sample.len()) / sample.len() + 1, sample.len());
    println!("\n============================================================\n");
    }
}
