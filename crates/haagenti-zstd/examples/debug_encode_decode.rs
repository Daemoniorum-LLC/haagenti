...
fn main() {
    ...
    for (i, enc) in encoded.iter().enumerate() {
        ...
        println!("       LL_extra={} ({} bits), OF_extra={} ({} bits), ML_extra={} ({} bits)",
    ...
    println!("\nInit with seq[{}] codes ({}, {}, {}): states = ({}, {}, {})",
             last_idx, last_seq.ll_code, last_seq.of_code, last_seq.ml_code,
             ll_init, of_init, ml_init);
    ...
    if last_seq.ll_bits > 0 {
        println!("  Write LL extra: {} ({} bits)", last_seq.ll_extra, last_seq.ll_bits);
        bits.write_bits(last_seq.ll_extra, last_seq.ll_bits);
    }
    if last_seq.ml_bits > 0 {
        println!("  Write ML extra: {} ({} bits)", last_seq.ml_extra, last_seq.ml_bits);
        bits.write_bits(last_seq.ml_extra, last_seq.ml_bits);
    }
    if last_seq.of_bits > 0 {
        println!("  Write OF extra: {} ({} bits)", last_seq.of_extra, last_seq.of_bits);
    ...
    println!("\nFinal bitstream: {:02x?} ({} bytes)", bitstream, bitstream.len());
    ...
    for i in 0..encoded.len() {
        ...
        if i < encoded.len() - 1 {
            ...
            println!("  Updated states: LL={}, OF={}, ML={}",
                     ll_decoder.state(), of_decoder.state(), ml_decoder.state());
        }
    }
