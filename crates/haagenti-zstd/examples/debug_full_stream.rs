...
fn main() {
    ...
    println!("=== Input: {:?} ({} bytes) ===\n", String::from_utf8_lossy(data), data.len());
    ...
    if compressed.len() >= 4 {
        ...
        if compressed.len() >= 5 {
            ...
            if single_segment == 0 { header_size += 1; } // Window_Descriptor
            header_size += match dict_id_flag { 0 => 0, 1 => 1, 2 => 2, _ => 4 };
            ...
            if compressed.len() > block_start + 3 {
                ...
                println!("\nBlock header at offset {}: 0x{:06x}", block_start, block_header);
                ...
                if block_type == 2 && compressed.len() > content_start {
                    ...
                    if compressed.len() > content_start {
                        ...
                        println!("  Literals_Block_Type: {} ({})", lit_type,
                            match lit_type { 0 => "Raw", 1 => "RLE", 2 => "Compressed", _ => "Treeless" });
