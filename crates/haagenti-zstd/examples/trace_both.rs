...
use haagenti_zstd::fse::{
    cached_ll_table, cached_of_table, cached_ml_table,
    FseBitWriter, FseDecoder, BitReader,
...
fn trace_case(name: &str, sequences: &[Sequence]) {
    ...
    for i in 0..last_idx {
        ...
        if seq.ll_bits > 0 { bits.write_bits(seq.ll_extra, seq.ll_bits); total_bits += seq.ll_bits as usize; }
        if seq.ml_bits > 0 { bits.write_bits(seq.ml_extra, seq.ml_bits); total_bits += seq.ml_bits as usize; }
        if seq.of_bits > 0 { bits.write_bits(seq.of_extra, seq.of_bits); total_bits += seq.of_bits as usize; }
        
        bits.write_bits(ll_fse.0, ll_fse.1); total_bits += ll_fse.1 as usize;
        bits.write_bits(ml_fse.0, ml_fse.1); total_bits += ml_fse.1 as usize;
        bits.write_bits(of_fse.0, of_fse.1); total_bits += of_fse.1 as usize;
    }
    
    // Last sequence
    if last_seq.ll_bits > 0 { bits.write_bits(last_seq.ll_extra, last_seq.ll_bits); total_bits += last_seq.ll_bits as usize; }
    if last_seq.ml_bits > 0 { bits.write_bits(last_seq.ml_extra, last_seq.ml_bits); total_bits += last_seq.ml_bits as usize; }
    if last_seq.of_bits > 0 { bits.write_bits(last_seq.of_extra, last_seq.of_bits); total_bits += last_seq.of_bits as usize; }
    
    bits.write_bits(ml_final, ml_log); total_bits += ml_log as usize;
    bits.write_bits(of_final, of_log); total_bits += of_log as usize;
    bits.write_bits(ll_final, ll_log); total_bits += ll_log as usize;
    
    let bitstream = bits.finish();
    println!("\nEncoded: {} data bits + 1 sentinel = {} bits, {} bytes",
             total_bits, total_bits + 1, bitstream.len());
    println!("Bitstream: {:02x?}", bitstream);
    println!("States (become decoder initial): LL={}, OF={}, ML={}", ll_final, of_final, ml_final);
    ...
    for i in 0..encoded.len() {
        ...
        println!("Seq {}: codes ({},{},{}) {} ({},{},{})",
                 ...
                 if ok { "==" } else { "!=" },
                 expected.ll_code, expected.of_code, expected.ml_code);
        
        if !ok { all_ok = false; }
        
        // Read extras
        if expected.ll_bits > 0 { let _ = reader.read_bits(expected.ll_bits as usize); }
        if expected.ml_bits > 0 { let _ = reader.read_bits(expected.ml_bits as usize); }
        if expected.of_bits > 0 { let _ = reader.read_bits(expected.of_bits as usize); }
