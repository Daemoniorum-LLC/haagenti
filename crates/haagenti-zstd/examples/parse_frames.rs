...
fn parse_frame(name: &str, data: &[u8]) {
    ...
    println!("  Magic: 0x{:08x} ({})", magic, 
             if magic == 0xFD2FB528 { "valid" } else { "INVALID" });
    ...
    if pos + 3 <= compressed.len() {
        ...
        println!("    Last={}, Type={} ({}), Size={}", last, btype,
                 match btype { 0 => "Raw", 1 => "RLE", 2 => "Compressed", _ => "Reserved" },
                 bsize);
        ...
        if btype == 2 && block_end <= compressed.len() {
            ...
            println!("      Type={} ({}), Size_Format={}", ltype,
                     match ltype { 0 => "Raw", 1 => "RLE", 2 => "Compressed", _ => "Treeless" },
                     sfmt);
            ...
            let (lit_hdr_size, lit_size) = if ltype <= 1 {
                match sfmt {
                    ...
                    _ => {
                        ...
                        let size = ((lh as usize >> 4) & 0xF) | ((b1 as usize) << 4) | ((b2 as usize) << 12);
                        (3, size)
                    }
                }
            } else {
            ...
            println!("      Header size: {}, Literal size: {}", lit_hdr_size, lit_size);
            ...
            if seq_start < compressed.len() {
                ...
                if seq_start + 1 < compressed.len() {
                    ...
                    println!("        LL_Mode={}, OF_Mode={}, ML_Mode={}", 
                             mode & 3, (mode >> 2) & 3, (mode >> 4) & 3);
                }
            }
        }
    }
