...
fn main() {
    for size in [1, 5, 11, 20, 100] {
        ...
        pos += match fcs_flag { 0 => if single_segment {1} else {0}, 1 => 2, 2 => 4, 3 => 8, _ => 0 };

        // Block header
        let block_header = u32::from_le_bytes([compressed[pos], compressed[pos+1], compressed[pos+2], 0]);
        let is_last = block_header & 1;
        let block_type = (block_header >> 1) & 3;
        let block_size = block_header >> 3;
        pos += 3;

        println!("Block: type={} ({}), size={}, last={}",
                 block_type,
        ...
        if block_type == 2 && pos < compressed.len() {
            ...
            println!("Literals: header=0x{:02x}, type={} ({}), size_format={}",
                     lit_header, lit_type,
