...
fn main() {
    ...
    let test_patterns: Vec<(&str, Vec<u8>)> = vec![
        ...
        ("2 unique bytes", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push(if i % 2 == 0 { b'A' } else { b'B' }); }
            v
        }),
        ("10 unique bytes", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push(b'0' + (i % 10) as u8); }
            v
        }),
        ("26 unique bytes (alpha)", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push(b'a' + (i % 26) as u8); }
            v
        }),
        ("64 unique bytes", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push(b' ' + (i % 64) as u8); }
            v
        }),
        ("128 unique bytes", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push((i % 128) as u8); }
            v
        }),
        ("256 unique bytes", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push((i % 256) as u8); }
            v
        }),
        ("random-looking but compressible", {
            let mut v = Vec::new();
            for i in 0..3000 { v.push(((i * 7 + 13) % 256) as u8); }
            v
        }),
        ("short log line (60 bytes)", b"[2024-01-01 10:00:00] INFO Processing request #12345\n".to_vec()),
        ("short log repeated", {
            let line = b"[2024-01-01 10:00:00] INFO Processing request #12345\n";
            let mut v = Vec::new();
            for _ in 0..50 { v.extend_from_slice(line); }
            v
        }),
    ...
    for (name, data) in test_patterns {
        ...
        match zstd::decode_all(Cursor::new(&compressed)) {
            Ok(dec) => {
                if dec == data {
                    println!("{:30} ({:5} bytes): OK ({:4} bytes)", name, data.len(), compressed.len());
                } else {
