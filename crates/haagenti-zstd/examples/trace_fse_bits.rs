...
fn decode_with_trace(seq_section: &[u8]) {
    ...
    println!("Initial states: LL={}, OF={}, ML={}", init_ll, init_of, init_ml);
    println!("Bits after states: {} (consumed {} for states)", bits.bits_remaining(), total_bits - bits.bits_remaining());

    // Switch to LSB mode
    bits.switch_to_lsb_mode().unwrap();
    println!("After LSB switch: {} bits at bit position", bits.bits_remaining());

    for i in 0..seq_count {
        let is_last = i == seq_count - 1;
        println!("\n--- Sequence {} {} ---", i, if is_last { "(LAST)" } else { "" });
        ...
        println!("  Extra bits needed: OF={}, ML={}, LL={}", of_extra_bits, ml_extra_bits, ll_extra_bits);
        ...
        println!("  Extras read: OF={}, ML={}, LL={}", of_extra, ml_extra, ll_extra);
        println!("  Bits after extras: {} (consumed {} for extras)", 
                 bits.bits_remaining(), before_extras - bits.bits_remaining());

        // Compute values
        let ll_value = get_ll_baseline(ll_code) + ll_extra as u32;
        let of_value = if of_code > 0 { (1u32 << of_code) + of_extra as u32 } else { of_extra as u32 };
        ...
        if !is_last {
            ...
            println!("  FSE update bits needed: LL={}, ML={}, OF={}", 
            ...
            println!("  Bits after FSE update: {} (consumed {} for FSE)", 
                     bits.bits_remaining(), before_update - bits.bits_remaining());
        }
    }
