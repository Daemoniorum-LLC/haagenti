...
use haagenti_zstd::fse::{
    FseBitWriter, InterleavedTansEncoder, BitReader, FseDecoder, FseTable,
...
fn main() {
    ...
    println!("  Verify: LL[{}]={}, OF[{}]={}, ML[{}]={}",
             ll_s, ll_table.decode(ll_s as usize).symbol,
             of_s, of_table.decode(of_s as usize).symbol,
             ml_s, ml_table.decode(ml_s as usize).symbol);
    ...
    println!("  FSE bits: LL=({}, {} bits), OF=({}, {} bits), ML=({}, {} bits)",
             ll_fse.0, ll_fse.1, of_fse.0, of_fse.1, ml_fse.0, ml_fse.1);

    let (ll_final, of_final, ml_final) = tans.get_states();
    println!("  Final states: LL={}, OF={}, ML={}", ll_final, of_final, ml_final);

    // Verify final states decode to Seq 0's codes
    println!("  Verify: LL[{}]={}, OF[{}]={}, ML[{}]={}",
             ll_final, ll_table.decode(ll_final as usize).symbol,
             of_final, of_table.decode(of_final as usize).symbol,
             ml_final, ml_table.decode(ml_final as usize).symbol);
    ...
    if last_seq.ll_bits > 0 {
        println!("  LL extra: {} ({} bits)", last_seq.ll_extra, last_seq.ll_bits);
        bits.write_bits(last_seq.ll_extra, last_seq.ll_bits);
    }
    if last_seq.ml_bits > 0 {
        println!("  ML extra: {} ({} bits)", last_seq.ml_extra, last_seq.ml_bits);
        bits.write_bits(last_seq.ml_extra, last_seq.ml_bits);
    }
    if last_seq.of_bits > 0 {
        println!("  OF extra: {} ({} bits)", last_seq.of_extra, last_seq.of_bits);
    ...
    println!("Expected: LL={}, OF={}, ML={}", ll_final, of_final, ml_final);
    println!("Match: {}", ll_state_read == ll_final && of_state_read == of_final && ml_state_read == ml_final);
    ...
    println!("\nSeq 0 symbols from states: LL={}, OF={}, ML={}", ll_sym, of_sym, ml_sym);
    println!("Expected: LL={}, OF={}, ML={}", seq0.ll_code, seq0.of_code, seq0.ml_code);

    // Switch to LSB mode for extras and FSE bits
    reader.switch_to_lsb_mode().unwrap();
    println!("\nSwitched to LSB mode. Bits remaining: {}", reader.bits_remaining());
    ...
    if seq0.ll_bits > 0 { reader2.read_bits(seq0.ll_bits as usize).unwrap(); }
    if seq0.ml_bits > 0 { reader2.read_bits(seq0.ml_bits as usize).unwrap(); }
    if seq0.of_bits > 0 { reader2.read_bits(seq0.of_bits as usize).unwrap(); }

    println!("\nAfter skipping Seq 0 extras, bits remaining: {}", reader2.bits_remaining());

    // Now update LL state
    println!("\nUpdating LL decoder:");
    let ll_entry = ll_table.decode(ll_state_cur);
    println!("  Current state: {}, num_bits to read: {}", ll_state_cur, ll_entry.num_bits);
    let ll_update_bits = reader2.read_bits(ll_entry.num_bits as usize).unwrap();
    println!("  Read {} bits: value={}", ll_entry.num_bits, ll_update_bits);
    let new_ll_state = ll_entry.baseline as usize + ll_update_bits as usize;
    ll_state_cur = new_ll_state;
    println!("  New state: {} (baseline {} + bits {})", new_ll_state, ll_entry.baseline, ll_update_bits);
    println!("  New symbol: {}", ll_table.decode(new_ll_state).symbol);

    // Update ML state
    println!("\nUpdating ML decoder:");
    let ml_entry = ml_table.decode(ml_state_cur);
    println!("  Current state: {}, num_bits to read: {}", ml_state_cur, ml_entry.num_bits);
    let ml_update_bits = reader2.read_bits(ml_entry.num_bits as usize).unwrap();
    println!("  Read {} bits: value={}", ml_entry.num_bits, ml_update_bits);
    let new_ml_state = ml_entry.baseline as usize + ml_update_bits as usize;
    ml_state_cur = new_ml_state;
    println!("  New state: {} (baseline {} + bits {})", new_ml_state, ml_entry.baseline, ml_update_bits);
    println!("  New symbol: {}", ml_table.decode(new_ml_state).symbol);

    // Update OF state
    println!("\nUpdating OF decoder:");
    let of_entry = of_table.decode(of_state_cur);
    println!("  Current state: {}, num_bits to read: {}", of_state_cur, of_entry.num_bits);
    let of_update_bits = reader2.read_bits(of_entry.num_bits as usize).unwrap();
    println!("  Read {} bits: value={}", of_entry.num_bits, of_update_bits);
    let new_of_state = of_entry.baseline as usize + of_update_bits as usize;
    of_state_cur = new_of_state;
    println!("  New state: {} (baseline {} + bits {})", new_of_state, of_entry.baseline, of_update_bits);
    println!("  New symbol: {}", of_table.decode(new_of_state).symbol);

    println!("\n=== ANALYSIS ===");
    println!("After Seq 0 FSE updates:");
    println!("  LL symbol: {} (expected {})", ll_table.decode(new_ll_state).symbol, last_seq.ll_code);
    println!("  OF symbol: {} (expected {})", of_table.decode(new_of_state).symbol, last_seq.of_code);
    println!("  ML symbol: {} (expected {})", ml_table.decode(new_ml_state).symbol, last_seq.ml_code);
