...
fn analyze_case(input: &[u8]) {
    ...
    let bh = u32::from_le_bytes([
        our_compressed[pos],
        our_compressed[pos + 1],
        our_compressed[pos + 2],
        0,
    ]);
...
fn decode_with_trace(bitstream: &[u8], seq_count: usize) {
    ...
    for i in 0..seq_count {
        ...
        println!(
            "    Reading extras in order: LL({} bits), ML({} bits), OF({} bits)",
        ...
        if i < seq_count - 1 {
            ...
            println!(
                "    FSE updates needed: LL({} bits), ML({} bits), OF({} bits)",
            ...
            println!(
                "    After FSE updates - Bits remaining: {}",
                bits.bits_remaining()
            );
            println!(
                "    New states: LL={}, OF={}, ML={}",
