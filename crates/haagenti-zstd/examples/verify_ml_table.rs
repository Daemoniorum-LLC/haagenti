...
fn main() {
    ...
    for state in 0..64 {
        ...
        if let Some(code) = ml_code {
            if code >= 40 {
                println!(
                    "  State {:2} → ML code {:2} (baseVal={:6}, {} extra bits)",
                    state, code, base_val, nb_add_bits
                );
            }
        } else {
            // For codes that don't directly match, find by baseVal
            let code_by_base = ml_baselines.iter().position(|&(base, _)| base == base_val);
            if let Some(c) = code_by_base {
                if c >= 40 {
                    println!(
                        "  State {:2} → ML code {:2} (baseVal={:6}, {} extra bits) [by base]",
    ...
    println!(
        "Reference state 42: nextState={}, nbAddBits={}, nbBits={}, baseVal={}",
    ...
    println!(
        "Need: 496 = 259 + extra → extra = 237 = 0x{:02x}",
        496 - 259
    );
