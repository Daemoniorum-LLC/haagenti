...
fn main() {
    ...
    let test_cases: Vec<(&str, Vec<u8>)> = vec![
        ...
        ("Pattern ABCD*250", (0..250).flat_map(|_| b"ABCD".iter().copied()).collect()),
        ("RLE-like", {
            let mut v = Vec::new();
        ...
        ("Text-like", "The quick brown fox jumps over the lazy dog. ".repeat(20).into_bytes()),
    ...
    for (name, data) in test_cases {
        ...
        println!("{:20} | Input: {:5} | Compressed: {:5} | Ratio: {:6.2}x | Correct: {}",
            name, data.len(), compressed.len(), ratio, if correct { "✓" } else { "✗" });
    }
