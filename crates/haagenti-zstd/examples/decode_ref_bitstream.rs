...
fn main() {
    ...
    println!("FSE bitstream: {} bytes = {:02x?}", fse_bitstream.len(), fse_bitstream);
    ...
    // Read initial states
    let ll_state = bits.read_bits(LITERAL_LENGTH_ACCURACY_LOG as usize).unwrap();
    ...
    for i in 0..seq_count {
        ...
        println!("Extras (OF,ML,LL order): OF={}, ML={}, LL={}", of_extra, ml_extra, ll_extra);
        ...
        if !is_last {
            ...
            println!("FSE bits needed: LL={}, ML={}, OF={}", ll_entry.num_bits, ml_entry.num_bits, of_entry.num_bits);
            println!("Bits available: {}", bits2.bits_remaining());
            ...
            for order in ["LL,ML,OF", "LL,OF,ML", "ML,LL,OF", "ML,OF,LL", "OF,LL,ML", "OF,ML,LL"] {
                ...
                if of_bits > 0 { bits_test.read_bits(of_bits as usize).ok(); }
                ...
                println!("    {}: LL={} (sym {}), OF={} (sym {}), ML={} (sym {}), remaining={}",
                         order, ll_test.state(), ll_sym, of_test.state(), of_sym, ml_test.state(), ml_sym,
