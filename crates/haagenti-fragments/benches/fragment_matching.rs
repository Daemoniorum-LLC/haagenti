...
use haagenti_fragments::{FragmentId, FragmentSignature, SignatureConfig};
...
fn bench_batch_operations(c: &mut Criterion) {
    ...
    for batch_size in batch_sizes {
        let fragments: Vec<_> = (0..batch_size)
            .map(|i| generate_tensor_data(fragment_size, i as u64))
            .collect();
        ...
        group.bench_with_input(
            ...
            |b, fragments| {
                b.iter(|| {
                    fragments
                        .iter()
                        .map(|data| FragmentSignature::compute(black_box(data), &config))
                        .collect::<Vec<_>>()
                })
            },
        );
...
fn bench_fragment_id(c: &mut Criterion) {
    ...
    group.bench_function("from_hex", |b| {
        b.iter(|| FragmentId::from_hex(black_box(&hex)))
    });
    group.finish();
}
