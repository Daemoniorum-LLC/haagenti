...
use haagenti_core::{Compressor, Decompressor, Error, Result};
...
impl<W: Write + Seek> HctWriter<W> {
    ...
    pub fn new(writer: W, algorithm: CompressionAlgorithm, dtype: DType, shape: Vec<u64>) -> Self {
        Self {
            writer,
            algorithm,
            dtype,
...
pub fn compress_file(
    ...
    // Read input
    let input_data = std::fs::read(input_path.as_ref())
    ...
    // Get output size
    let output_metadata = std::fs::metadata(output_path.as_ref())
...
impl<R: Read + Seek> HctReaderV2<R> {
    ...
    pub fn new(mut reader: R) -> Result<Self> {
        ...
        let index_entry_size =
            if version >= HCT_VERSION_V2 && header.flags & FLAG_BLOCK_CHECKSUMS != 0 {
                BlockIndexV2::SIZE
            } else {
                BlockIndex::SIZE
            };
        ...
        let quant_size = if quantization.is_some() {
    ...
    pub fn decompress_all_validated(
        &mut self,
        decompressor: &impl Decompressor,
    ) -> Result<Vec<u8>> {
        let mut output = Vec::with_capacity(self.header.original_size as usize);
...
mod tests {
    ...
    fn test_all_dtypes_roundtrip() {
        let dtypes = [DType::F32, DType::F16, DType::BF16, DType::I8, DType::I4];
    ...
    fn test_all_algorithms_roundtrip() {
        let algorithms = [CompressionAlgorithm::Lz4, CompressionAlgorithm::Zstd];
    ...
    fn test_reader_invalid_block_index_bounds() {
...

        {
            ...
            let codec = haagenti_lz4::Lz4Codec::new();
    ...
    fn test_v2_checksum_validation_detects_bitflip() {
...

        {
            ...
            let codec = haagenti_zstd::ZstdCodec::new();
            writer.compress_data(&data, &codec).unwrap();
            writer.finish().unwrap();
        }
    ...
    fn test_writer_multiple_compressions() {
        ...
        let cursor = std::io::Cursor::new(&mut output);
        let mut writer = HctWriter::new(cursor, CompressionAlgorithm::Lz4, DType::F32, vec![100])
            .with_block_size(64);
    ...
    fn test_decompression_with_wrong_algorithm() {
        ...
        let lz4 = haagenti_lz4::Lz4Codec::new();
