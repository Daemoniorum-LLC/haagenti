...
use haagenti::holotensor::{HoloTensorReader, HoloTensorDecoder};

fn main() {
    // Test inline format model (total_fragments=0 in header but data present)
    let path = std::env::args().nth(1)
        .unwrap_or_else(|| "/home/crook/models/smollm2-135m-hct3/model.embed_tokens.weight.hct".to_string());
    ...
    match HoloTensorReader::new(reader) {
        Ok(mut holo_reader) => {
            ...
            println!("  Total fragments (effective): {}", holo_reader.total_fragments());
            ...
            match holo_reader.read_all() {
                Ok((header, fragments)) => {
                    ...
                    for frag in fragments {
                        println!("  Fragment {}: {} bytes, flags={}", frag.index, frag.data.len(), frag.flags);
